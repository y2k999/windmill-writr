 ...
 * 
 * @since 1.0.1
 * 	Copy from original archive template ([Parent]/template/content/index.php) and modify it.
	...
	<div class="uk-grid uk-grid-small uk-child-width-expand"<?php echo apply_filters("_attribute[grid]",''); ?>>
		...
		<main id="primary" class="site-main"<?php echo apply_filters("_attribute[column][primary]",''); ?>>
			...
			*/
			...
			?>
			<div class="uk-grid-small uk-grid-match uk-child-width-1-1"<?php echo apply_filters("_attribute[grid]",''); ?>>
				<?php
				while(have_posts()) :	the_post();
					get_template_part('template-part/content/content','archive');
				endwhile;
				?>
			</div><!-- .grid -->
